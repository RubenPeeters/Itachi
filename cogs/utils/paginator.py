...
class EmotePaginator(Pages):
    def __init__(self, ctx, entries, *, per_page=9):
        super().__init__(ctx, entries=entries, per_page=per_page)
        self.total = len(entries)
        self.title = 'Emotes in database <a:ameowcoffee:464806960638853121>'
        self.description = "Stored in personal emote guilds"
        self.prefix = cleanup_prefix(ctx.bot, ctx.prefix)
    ...
    async def show_page(self, page, *, first=False):
        ...
        if hasattr(self, '_is_bot'):
            value ='For more help, join my support server: https://discord.gg/2XfmHUH <:blobwave:464513909374582784>'
            self.embed.add_field(name='Support', value=value)

        self.embed.set_footer(text=f'Use "{self.prefix}help command" for more info on how to use the emotedatabase.')


        for entry in entries:
            self.embed.add_field(name="\u200b", value=entry)

        if self.maximum_pages:
            self.embed.set_author(name=f'Page {page}/{self.maximum_pages} ({self.total} emotes)')
